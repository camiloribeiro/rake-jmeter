namespace :servers do
  ...
  STRESS_SERVERS_EXTERNAL.each do |machine|
      ...
      cmd = "ssh #{machine.name} '~/#{@project_dir}/jmeter/#{@jmeter_version}/bin/jmeter-server -n -p ~/#{@project_dir}/#{args.properties_file} -Jhostname=#{@hostname} -Jport=#{@port}' 2>&1 >> /dev/null < /dev/null & "
